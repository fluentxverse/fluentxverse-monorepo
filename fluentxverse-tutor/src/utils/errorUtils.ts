...
function humanizeErrorMessage(message: string): string {
  ...
  const mappings: [RegExp | string, string][] = [
    ...
    [/tutor.*not.*available/i, 'This tutor is not available at the selected time.'],
    [/schedule.*conflict/i, 'There is a scheduling conflict. Please choose a different time.'],
  ];
